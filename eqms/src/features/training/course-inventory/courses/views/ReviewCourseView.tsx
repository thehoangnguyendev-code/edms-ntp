...
import { courseReview } from "@/components/ui/breadcrumb/breadcrumbs.config";
...
export const ReviewCourseView: React.FC = () => {
  ...
  const { canReviewCourses } = useTrainingPermissions();
  ...
  const canAct = effectiveStatus === "Pending Review" && canReviewCourses;
  ...
  return (
    <div className="space-y-4 md:space-y-6 w-full flex-1 flex flex-col">
      ...
      <PageHeader
        title="Review Course"
        breadcrumbItems={courseReview(navigate, location.state?.from)}
        actions={
          <>
            <Button
