...
import {
  COURSE_AUDIT_ACTION_OPTIONS,
  COURSE_AUDIT_MOCK_DATA,
  type CourseAuditEntry,
} from "./CourseAuditMockData";
import { formatAuditActionLabel } from "@/features/audit-trail/utils/actionBadge";

interface CourseAuditTrailTabProps {
  mockData?: CourseAuditEntry[];
  actionOptions?: SelectOption[];
  emptyMessage?: string;
}

export const CourseAuditTrailTab: React.FC<CourseAuditTrailTabProps> = ({
  mockData = COURSE_AUDIT_MOCK_DATA,
  ...
  const [selectedEntry, setSelectedEntry] = useState<CourseAuditEntry | null>(null);
  ...
  const getActionBadge = (actionType: string) => {
    ...
    return styles[actionType] || "bg-slate-50 text-slate-600 border-slate-200";
  };

  const openChangesModal = (entry: CourseAuditEntry) => {
