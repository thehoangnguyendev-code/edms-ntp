...
import { Users, Trash2, Search, User, Check, GripVertical, Plus } from "lucide-react";
import { Badge } from "@/components/ui/badge/Badge";
import { Button } from "@/components/ui/button/Button";
import { cn } from "@/components/ui/utils";
import { FormModal } from "@/components/ui/modal/FormModal";
import { metadataApi } from "@/services/api/metadata";
import type { MaterialWorkflowUser } from "@/features/training/materials/views/materialWorkflowUsers";
import type { CourseReviewer } from "./workflow.types";

interface CourseReviewersTabProps {
  reviewers: CourseReviewer[];
  onReviewersChange: (nextReviewers: CourseReviewer[]) => void;
  users: MaterialWorkflowUser[];
  readOnly?: boolean;
}
...
interface UserSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (users: MaterialWorkflowUser[]) => void;
  existingIds: string[];
  users: MaterialWorkflowUser[];
}

export const ReviewerSelectionModal: React.FC<UserSelectionModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  existingIds,
  ...
  const [selectedUsersById, setSelectedUsersById] = useState<Record<string, MaterialWorkflowUser>>({});
  const [searchResults, setSearchResults] = useState<MaterialWorkflowUser[] | null>(null);
  ...
  const handleSave = () => {
    const selectedUsers = selectedIds
      ...
      .filter((u): u is MaterialWorkflowUser => !!u);
  ...
  return (
    <FormModal
      ...
      description="Select users who will review this course."
...
export const CourseReviewersTab: React.FC<CourseReviewersTabProps> = ({
  reviewers,
  onReviewersChange,
  users,
  readOnly = false,
  ...
  if (readOnly) {
    const sortedReviewers = [...reviewers].sort((a, b) => a.order - b.order);
    const rows = sortedReviewers.length > 0
      ? sortedReviewers
      : [{ id: "empty", fullName: "", order: 1 } as CourseReviewer];
                    ...
                    >
                      ...
                      <td className="py-2 px-2 sm:py-3.5 sm:px-4 text-xs sm:text-sm whitespace-nowrap">
                        <div className="flex items-center gap-1.5 sm:gap-2">
                          {!readOnly && <GripVertical className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-slate-400" />}
                          <span className="inline-flex items-center justify-center h-5 w-5 sm:h-6 sm:w-6 rounded-full bg-emerald-50 text-emerald-700 text-2xs font-bold">
                            {reviewer.order}
                          </span>
                        </div>
                      </td>
