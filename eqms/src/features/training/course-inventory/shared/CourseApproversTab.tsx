...
import { CheckCircle, Trash2, Search, User, Check, Plus } from "lucide-react";
...
import type { CourseApprover } from "./workflow.types";

interface CourseApproversTabProps {
  approvers: CourseApprover[];
  onApproversChange: (nextApprovers: CourseApprover[]) => void;
...
interface UserSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (user: MaterialWorkflowUser) => void;
  users: MaterialWorkflowUser[];
}

export const ApproverSelectionModal: React.FC<UserSelectionModalProps> = ({
  isOpen,
  onClose,
  onSelect,
  users,
  ...
  return (
    <FormModal
      ...
      description="Select the final approver for this course."
...
export const CourseApproversTab: React.FC<CourseApproversTabProps> = ({
  approvers,
  onApproversChange,
  users,
  readOnly = false,
  ...
  const handleSelectUser = (user: MaterialWorkflowUser) => {
  ...
  if (readOnly) {
    const rows = approvers.length > 0
      ? approvers
      : [{ id: "empty", fullName: "" } as CourseApprover];
