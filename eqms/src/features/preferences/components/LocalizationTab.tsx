...
export const LocalizationTab: React.FC<LocalizationTabProps> = ({ onRegisterSaveHandler, onRegisterResetHandler }) => {
  ...
  useEffect(() => {
    void localizationApi.getSystem().then(value => setSystem({ ...DEFAULT_LOCALIZATION_SETTINGS, ...value })).catch(() => setSystem(readSystemLocalizationSettings));
    void localizationApi.getMine().then(setPreferences).finally(() => setLoading(false));
  }, []);

  const effective = { ...DEFAULT_LOCALIZATION_SETTINGS, ...(preferences.useSystemDefaults ? system : { ...system, ...preferences }) };
  const setValue = <K extends keyof SystemLocalizationSettings>(key: K, value: SystemLocalizationSettings[K]) => setPreferences(current => ({ ...current, useSystemDefaults: false, [key]: value }));

  const saveChanges = useCallback(async () => {
    const saved = await localizationApi.updateMine(preferences.useSystemDefaults ? { useSystemDefaults: true } : preferences);
