...
const buildEmailTemplateDelta = (
  original: EmailTemplate | null,
  next: EmailTemplateFormData,
): Partial<EmailTemplatePayload> | null => {
  if (!original) {
    return next;
  }

  const delta: Partial<EmailTemplatePayload> = {};
  const fields: (keyof EmailTemplateFormData)[] = [
    "name",
    "subject",
    "type",
    "description",
  ...
  fields.forEach((field) => {
    const originalValue = original[field as keyof EmailTemplate];
    const nextValue = next[field];

    if (JSON.stringify(originalValue ?? null) !== JSON.stringify(nextValue ?? null)) {
      (delta as Record<string, unknown>)[field] = nextValue;
    }
  });
...
export const EmailTemplateEditView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadTemplate = async () => {
      ...
      try {
        const template = await emailTemplateApi.getEmailTemplate(id);
        if (cancelled) {
          return;
        }
          ...
          const data = {
            ...
            primaryColor: template.primaryColor || BRAND_PRIMARY_HEX,
        ...
        if (!cancelled) {
          ...
          setLoadError("Unable to load the selected email template.");
        }
      }
    };
  ...
  const handleESignConfirm = async (data: { reason: string; signatureToken?: string }) => {
    ...
    try {
      const delta = buildEmailTemplateDelta(originalData, formData);
      if (!delta) {
