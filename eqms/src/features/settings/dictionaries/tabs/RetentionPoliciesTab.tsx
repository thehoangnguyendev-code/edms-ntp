...
export const RetentionPoliciesTab = React.forwardRef<{ openAddModal: () => void }, {}>((_, ref) => {
  ...
  const {
    ...
    handleSort,
    reload,
  } = useDictionaryServerTable<RetentionPolicyItem>({
    fetcher: dictionaryApi.getRetentionPoliciesPage,
    defaultSortBy: "name",
    extraParams: {
      status: statusFilter,
  ...
  const handleSave = async (payload: RetentionPolicyFormData) => {
      try {
        let saved;
        if (showEditModal && selectedItem) {
        saved = await dictionaryApi.updateRetentionPolicy(selectedItem.id, payload);
      } else {
        saved = await dictionaryApi.createRetentionPolicy(payload);
      }

      openResultModal(
        "success",
        showEditModal ? "Retention Policy Updated" : "Retention Policy Created",
        `Retention policy "${saved.name}" has been ${showEditModal ? "updated" : "created"} successfully.`
      );
      reload();
      ...
      openResultModal(
        "error",
        showEditModal ? "Retention Policy Update Failed" : "Retention Policy Create Failed",
        extractApiMessage(error, showEditModal ? "Unable to update retention policy." : "Unable to create retention policy.")
      );
      throw error;
    }
  };

  const handleRequestToggleStatus = (item: RetentionPolicyItem) => {
  ...
  const handleConfirmToggleStatus = async () => {
    ...
    try {
      const updated = await dictionaryApi.updateRetentionPolicy(item.id, {
        name: item.name,
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      await dictionaryApi.deleteRetentionPolicy(selectedItem.id);
  ...
  return (
    <div className="flex-1 flex flex-col gap-4 md:gap-5 min-h-0">
          ...
          <table className="w-full">
            <thead className="sticky top-0 z-30">
              <tr>
                ...
                {[
                  { label: "Retention Policy Name", id: "name" },
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="py-14 text-center text-slate-500">Loading retention policies...</td>
                </tr>
              ) : items.length > 0 ? (
      ...
      <AlertModal
        ...
        type="warning"
        title={`${selectedItem?.isActive ? "Deactivate" : "Activate"} Retention Policy?`}
        description={
          <div className="space-y-3">
            <p>
...
interface RetentionPolicyModalProps {
  ...
  isEdit: boolean;
  onSave: (data: RetentionPolicyFormData) => Promise<void>;
}

const RetentionPolicyModal: React.FC<RetentionPolicyModalProps> = ({ isOpen, onClose, item, isEdit, onSave }) => {
  ...
  return (
    <FormModal isOpen={isOpen} onClose={onClose} onConfirm={handleSubmit} title={`${isEdit ? "Edit" : "Add New"} Retention Policy`} isLoading={isSaving} size="lg">
      <div className="space-y-4">
        <div>
          <label className="block text-xs sm:text-sm font-medium text-slate-700 mb-1.5">
            Retention Policy Name<span className="text-red-500 ml-1">*</span>
          </label>
          <input
            type="text"
            value={formData.name}
        ...
        <div>
          <label className="block text-xs sm:text-sm font-medium text-slate-700 mb-1.5">Retention period (days)</label>
          <input
            type="number"
            min={1}
            value={formData.retentionDays ?? ""}
            onChange={(e) => setFormData({ ...formData, retentionDays: e.target.value === "" ? null : Number(e.target.value) })}
            className="w-full h-9 px-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500"
            placeholder="Leave blank for permanent retention"
            disabled={isSaving}
          />
          <p className="mt-1 text-xs text-slate-500">Leave blank only when the policy is intended to retain records permanently.</p>
        </div>
